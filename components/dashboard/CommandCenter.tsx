...
export default function CommandCenter() {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Log system errors from Black Box Recorder
      if (result.systemErrors && result.systemErrors.length > 0) {
        result.systemErrors.forEach((error: string) => {
      ...
      if (result.verdict) {
        addLog(
          ...
          result.verdict === "MALICIOUS" ? "WARNING" : "SUCCESS"
        );
      } else {
        // Log successful scan
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-panel rounded-lg p-6">
        <h2 className="text-xl font-bold tracking-wide mb-4">IOC SCANNER</h2>
